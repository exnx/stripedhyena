...
class RotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, base=10000, precision=torch.half):
        ...
        self.precision = precision
        # storing dim and base for compatibility with new RoPEs (see below):
        self.dim = dim
        self.base = base
...
class RotaryEmbedding2(torch.nn.Module):
    ...
    Args:
        ...
        base (float): base for the frequency. Default: 10000
        precision (torch.dtype): precision of the embedding. Default: torch.half
    """
    def __init__(self,
        dim: int,
        base: float=10000.,
        ...
        self.dim = dim
        self.base = base
        self._set_inv_freq(base=base)

    def _set_inv_freq(self, base: float):
        inv_freq = 1.0 / (base ** (torch.arange(0, self.dim, 2).float() / self.dim))
        self.register_buffer("inv_freq", inv_freq)

    def _set_cos_sin_cached(self, x, seq_len=None, t=None):
        if t is None:
            t = torch.arange(seq_len, device=x.device).type_as(self.inv_freq)
        else:
        ...
        freqs = torch.einsum("i,j->ij", t, self.inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1).to(x.device)
        emb = emb.float() if self.precision == torch.bfloat16 else emb
        self.cos_cached = emb.cos()[:, None, None, :]
        self.sin_cached = emb.sin()[:, None, None, :]
        if self.precision == torch.bfloat16:
            self.cos_cached = self.cos_cached.bfloat16()
    ...
    def _update(self, x, seq_len):
        self.seq_len_cached = seq_len
        self._set_cos_sin_cached(x=x, seq_len=seq_len)

    def forward(self, x, seq_dim=1, seq_len=None):
        seq_len = x.shape[seq_dim] if seq_len is None else seq_len
        if seq_len != self.seq_len_cached:
            self._update(x=x, seq_len=seq_len)
        return self.cos_cached, self.sin_cached


class LinearlyScaledRotaryEmbedding(RotaryEmbedding2):
    r"""
    Linearly scaled rotary position embedding.
    This interpolation scheme linearly interpolates the fourier space.

    Credit goes to Reddit user /u/kaiokendev.

    Args:
        dim (int): embedding dimension
        scaling_factor (float). Default: 1 (no scaling)
        base (float): base for the frequency. Default: 10000
        precision (torch.dtype): precision of the embedding. Default: torch.half
    """
    def __init__(self,
        ...
        super().__init__(dim=dim, base=base, precision=precision)
    ...
    def _update(self, x, seq_len):
        ...
        self._set_cos_sin_cached(x=x, seq_len=seq_len, t=t)


class NTKScaledRotaryEmbedding(RotaryEmbedding2):
    r"""
    Rotary position embedding with dynamic neural tangent kernel (NTK) scaling.
    In contrast to linear RoPE scaling, this interpolation scheme changes the base
    of the RoPE instead of the scale.

    Credit goes to Reddit users /u/bloc97 and /u/emozilla.

    Args:
        dim (int): embedding dimension
        max_unscaled_seq_len (int): maximum unscaled sequence length. Default: 2048
        scaling_factor (float): Default: 1 (no scaling)
        base (float): base for the frequency. Default: 10000
        precision (torch.dtype): precision of the embedding. Default: torch.half
    """
    def __init__(self,
        ...
        super().__init__(dim=dim, base=base, precision=precision)
        self.scaling_factor = scaling_factor
        self.max_unscaled_seq_len = max_unscaled_seq_len

    def _scale_base(self, seq_len):
        return self.base * (
            (self.scaling_factor * seq_len / self.max_unscaled_seq_len) - (self.scaling_factor - 1)
        ) ** (self.dim / (self.dim - 2))

    def _update(self, x, seq_len):
        self.seq_len_cached = seq_len

        if seq_len > self.max_unscaled_seq_len:
            base = self._scale_base(seq_len=seq_len)
            self._set_inv_freq(base=base)
...
if __name__ == '__main__':
    ...
    # testing RoPE fwd:
    rope = RotaryEmbedding(dim=512, base=10000)
    rope2 = RotaryEmbedding2(dim=512, base=10000)
    rope_linear = LinearlyScaledRotaryEmbedding(dim=512, scaling_factor=1.0, base=10000)
    rope_ntk = NTKScaledRotaryEmbedding(dim=512, max_unscaled_seq_len=2048, scaling_factor=1.0, base=10000)
    ...
    y3 = rope_linear(x)
    y4 = rope_ntk(x)
    assert all(torch.allclose(a,b) for a,b in zip(y1, y2))
    assert all(torch.allclose(a,b) for a,b in zip(y1, y3))
    assert all(torch.allclose(a,b) for a,b in zip(y1, y4))

    # testing AliBi fwd:
    alibi = AliBi(num_heads=16, mp_size=1, mp_rank=0)
    x = torch.randn(8, 16, 1024, 1024)
    y = alibi(x)